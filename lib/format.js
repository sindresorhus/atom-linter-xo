...
export default function format(editor) {
	...
	return report => {
		...
		return messages.map(message => {
			const [err, range = []] = withErrorHandling(selectMessageRange)(editor, message);
			const html = selectMessageHtml(message, err);
...
function selectMessageRange(editor, message) {
	...
	try {
		...
		throw new Error(`Failed getting range ${start.join(':')}. This is most likely an issue with ESLint. ${err.message}`);
	}
}
...
// (task: Function) => (...args) => [err: null|Error, value: any]
function withErrorHandling(task) {
	return (...args) => {
		try {
			return [null, task(...args)];
		} catch (err) {
			return [err];
		}
	};
